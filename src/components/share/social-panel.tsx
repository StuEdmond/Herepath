...
import type { SocialApp } from "@/lib/native";
import { BrandIcon } from "./brand-icon";

/** The longest caption each app accepts. */
export const CAPTION_LIMITS: Record<SocialApp, number> = { instagram: 2200, tiktok: 4000 };

const NAMES: Record<SocialApp, string> = { instagram: "Instagram", tiktok: "TikTok" };
...
interface SocialPanelProps {
  ...
  /** True on a phone or in the app, where the share sheet can hand the image straight to the app. */
...
export function SocialPanel({
  ...
  // TikTok is always the tall format; Instagram lets the rider choose a post or a story.
  const story = tiktok || format === "story";
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <h2 className="flex items-center gap-2 text-[18px] text-text-primary">
          <BrandIcon brand={network} className="h-5 w-5" /> {name}
        </h2>
      ...
      {tiktok ? (
        <div className="relative mx-auto aspect-[9/16] w-full max-w-[220px] overflow-hidden rounded-xl border border-text-muted/30 bg-bg">
          {previewUrl && (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={previewUrl} alt="How your TikTok post will look" className="h-full w-full object-cover" />
          )}
          <div className="absolute bottom-3 right-2 flex flex-col items-center gap-3 text-white drop-shadow" aria-hidden="true">
            <Heart className="h-6 w-6" />
            <MessageCircle className="h-6 w-6" />
            <Bookmark className="h-6 w-6" />
            <Send className="h-6 w-6" />
          </div>
          <span className="absolute bottom-3 left-2 text-[13px] font-medium text-white drop-shadow" aria-hidden="true">
            @your_username
          </span>
        </div>
      ) : (
      ...
      {over && <p className="-mt-2 text-[12px] text-red-500">{name} won&apos;t accept a caption this long. Shorten it before you post.</p>}

      <ol className="list-decimal space-y-1 pl-5 text-[13px] text-text-secondary">
        {canSendToApp ? (
          tiktok ? (
            <>
              <li>Tap Send to TikTok and follow its prompts to post the image.</li>
              <li>Paste the caption when it asks for one. It&apos;s copied for you, and TikTok can&apos;t fill it in itself.</li>
            </>
          ) : (
            <>
          ...
          <>
            ...
            <li>Open TikTok&apos;s upload page and choose the saved image.</li>
            <li>Paste the caption and post. TikTok&apos;s website is built for video, so photo posts are often easier from your phone.</li>
          </>
        ) : (
          <>
            <li>Save the image and copy the caption.</li>
      ...
      <div className="flex flex-col gap-2">
        <Button type="button" onClick={onSend} className="min-h-11">
          <BrandIcon brand={network} className="h-5 w-5" /> {canSendToApp ? `Send to ${name}` : `Open ${name}`}
        </Button>
        <div className="grid grid-cols-2 gap-2">
