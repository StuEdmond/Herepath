...
- **Google:** Google's Maps URL documentation allows 3 waypoints on mobile browsers and 9 otherwise. `src/lib/map-links.ts` sends 8 per leg (plus a start and a finish). **Tested by the owner (September 2026):** on the routes checked, Google Maps showed up to 8 steps, so the 8-waypoint legs are being honoured and no change is needed. Still worth confirming once: whether the 8 counted the stops between start and finish (all kept) or included them (two dropped), and whether it was the Google Maps app or a browser. The buttons still say "approximate" because Google picks its own roads between waypoints.
